// Wrapped P&L App Script
window.initPnLApp = function () {
    console.log("Starting P&L App inside Backtest Viewer");
    // const pnlTab = document.getElementById('tab-pnl-report');
    // if (!pnlTab) return; // Removed during navigation flattening

    // Scope helpers (if needed) or just run existing logic assuming unique IDs
    (function () {
        ...
        // --- View Toggle Logic (Handled Globally Now) ---
        // Expose Update Functions
        window.pnlUpdateDashboard = function () {
            if (viewDashboard) viewDashboard.classList.remove('hidden'); // Ensure visible if needed
            updateDashboard();
        };

        window.pnlUpdateMonthly = function () {
            if (viewMonthly) viewMonthly.classList.remove('hidden');
            updateMonthlyReport();
        };

        window.pnlLoadPortfolio = function () {
            if (viewPortfolio) viewPortfolio.classList.remove('hidden');
            loadPortfolioRows();
        };

        // Initial Load (default visible tab handled by index.html)
        // No listener binding needed for these internal buttons anymore.
        ...
        function initChart() {
            ...
            try {
                ...
                const elGlobalEquity = document.getElementById('globalOwnerEquityChart');
                if (elGlobalEquity) {
                    const ctxGlobal = elGlobalEquity.getContext('2d');
                    globalOwnerEquityChart = new Chart(ctxGlobal, {
                        type: 'line',
        ...
        function updateGlobalOwnerEquityChart() {
            if (!globalOwnerEquityChart) initChart();
            if (!globalOwnerEquityChart) return;

            const trs = document.getElementById('table-body').querySelectorAll('tr');
            const ownerDailyStats = {}; // { owner: { date: pl } }
                ...
                if (inputs.length >= 5) {
                    ...
                    if (exitDateStr && !isNaN(pl)) {
                        ...
                        if (exitDate && exitDate.getFullYear() === currentYear && exitDate.getMonth() === currentMonth) {
                            ...
                            ownerDailyStats[owner][exitDateStr] = (ownerDailyStats[owner][exitDateStr] || 0) + pl;
                            allDatesSet.add(exitDateStr);
                        }
                    }
                }
            });
            ...
            const datasets = [];
            Object.keys(ownerDailyStats).forEach(owner => {
                const color = getColorForString(owner);
                let cumulative = 0;
                const ownerData = sortedDates.map(date => {
                    cumulative += (ownerDailyStats[owner][date] || 0);
                    return cumulative;
                });
            ...
            globalOwnerEquityChart.data.labels = sortedDates;
            globalOwnerEquityChart.data.datasets = datasets;
            globalOwnerEquityChart.update();
        }
            ...
            ['owners', 'types'].forEach(key => {
                ...
                data.forEach(item => {
                    ...
                    if (!uniqueMap.has(lower)) {
                        uniqueMap.set(lower, trimmed); // Keep first encountered casing
                    }
                });

                data = Array.from(uniqueMap.values()).sort((a, b) => a.localeCompare(b));
                appData[key] = data;
                saveData(key);
            });
        ...
        function addItemIfNew(key, newItem) {
            ...
            if (trimmed) {
                const exists = appData[key].some(item => item.toLowerCase() === trimmed.toLowerCase());
                if (!exists) {
                    appData[key].push(trimmed);
                    appData[key].sort((a, b) => a.localeCompare(b));
                    saveData(key);
                }
            }
        }
        ...
        function createGlobalDropdown() {
            ...
            wrapper.className = 'pnl-theme-wrapper';
            wrapper.style.position = 'absolute';
            wrapper.style.top = '0';
            wrapper.style.left = '0';
            wrapper.style.width = '0';
            wrapper.style.height = '0';
            wrapper.style.padding = '0'; // Fix: prevent padding area from showing background
            wrapper.style.backgroundColor = 'transparent'; // Fix: prevent background from showing
            wrapper.style.zIndex = '10001'; // Ensure it's on top of everything
            ...
            globalDropdown.style.width = 'auto'; // Reset width

            wrapper.appendChild(globalDropdown);
        ...
        function positionDropdown(input) {
            ...
            globalDropdown.style.top = `${rect.bottom + window.scrollY + 4}px`;
        ...
        function renderDrillDownChart(trades) {
            const canvas = document.getElementById('details-chart');
            if (!canvas) return;

            // Sort trades by date for a correct equity curve
            const sortedTrades = [...trades].sort((a, b) => {
                const d_a = parseRobustDate(a.exitDate !== '--' ? a.exitDate : a.date) || new Date(0);
                const d_b = parseRobustDate(b.exitDate !== '--' ? b.exitDate : b.date) || new Date(0);
                return d_a - d_b;
            });
            ...
            // Update Metrics HUD
            const elPl = document.getElementById('details-metric-pl');
            const elMdd = document.getElementById('details-metric-mdd');
            const elWin = document.getElementById('details-metric-winrate');
            const elPf = document.getElementById('details-metric-pf');
            ...
            const labels = sortedTrades.map(t => t.exitDate !== '--' ? t.exitDate : t.date);
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function (context) {
                                    return 'Cum P/L: ₹' + context.parsed.y.toLocaleString('en-IN', { minimumFractionDigits: 2 });
                                }
                            }
                        }
                    },
